...
## Rendering a single template with all elements

A template can be configured to render a single template with all elements by setting `.single: true`.

When configured this way, the template is only rendered once, but it will receive all elements from the generators in a single value.

This can be used to combine a set of generated elements into a single resource.
  ...
  generators:
    - list:
        elements:
          ...
          - name: cluster3
  templates:
    # This configures the template not to be repeated.
    - single: true
      content:
        apiVersion: v1
        kind: ConfigMap
        metadata:
          ...
          namespace: default
        data:
          names: "{{- range .Elements }}{{ .name }},{{- end }}"
```

**Note** that the elements are within the `.Elements` variable in the template.

This generates a single `ConfigMap` with the `name` fields output in the data.
...
metadata:
  labels:
    sets.gitops.pro/name: single-element-test
    sets.gitops.pro/namespace: default
  name: test-configmap
  namespace: default
data:
  names: cluster1,cluster2,cluster3,
```

## Raw templates

Using the [repeating mechanism](https://pkg.go.dev/text/template#hdr-Variables) in the templating language can lead to cases where the desired template is not valid YAML.

In this case, you can opt to use a raw template instead of the normal content field.
...
spec:
  ...
  templates:
    - single: true
      # Note that this is a multi-line YAML string
      raw: |
        ...
        metadata:
          name: read-secrets
          namespace: demo
        # This iterates over each element in the list generator.
        subjects:
          {{- range .Elements }}
          - kind: User
            name: {{ .username }}
...
This uses [YAML multi-line](https://yaml-multiline.info/) formatting to write the template.

All elements are provided in the `.Elements` field that are generated by each generator, separate generators are not passed in, you can use a matrix to combine multiple generators.

In this case, a single `RoleBinding` is generated, which references three `subjects` one for each user in the list generator.

As always, any generator can be used and the generated resources will be exposed to the templating mechanism.
